...
Once you've set up your organisation account on the [Yoti Hub](https://hub.yoti.com/logout), you’re ready to start integrating with the Age estimation and anti-spoofing (liveness) API.
...
For optimal results, Yoti provides the [Yoti Face Capture modules,](https://www.npmjs.com/package/@getyoti/react-face-capture) which are designed to capture optimal images meeting the requirements of the API. This is available as a Native package, and Web implementation.
...
<div class="alert-GTK">
    ...
    <div class="alert-text">
      Please contact Yoti if you opt to not use the Face capture module. 
    </div>
    <div class="alert-links"> 
...
| ---- | ---- | 
| [https://api.yoti.com/ai/v1/age?secure=true](https://api.yoti.com/ai/v1/age) | Use Yoti's Age estimation service. | 
| [https://api.yoti.com/ai/v1/antispoofing?secure=true](https://api.yoti.com/ai/v1/antispoofing) | Use Yoti's Anti-spoofing (liveness) check. | 
| [https://api.yoti.com/ai/v1/age-antispoofing?secure=true](https://api.yoti.com/ai/v1/age-antispoofing) | Use Yoti's Age estimation service and the Anti-spoofing (liveness) check. | 
{% /table %}
...
private final KeyPairSource fileKeyPairSource;
...
this.fileKeyPairSource = fromFile(new File("<PEM_FILE_PATH>"));
this.keyPair = fileKeyPairSource.getFromStream(new KeyStreamVisitor());

JSONObject body = new JSONObject();
body.put("img", requestPayload.get("img"));
body.put("metadata", requestPayload.get("metadata")); // Optional, but recommended
body.put("secure", requestPayload.get("secure"));
byte[] payload = body.toString().getBytes(java.nio.charset.StandardCharsets.UTF_8);

SignedRequestBuilderFactory signedRequestBuilderFactory = new SignedRequestBuilderFactory();
SignedRequest signedRequest = signedRequestBuilderFactory.create()
    .withKeyPair(keyPair)
    .withBaseUrl("https://api.yoti.com/ai/v1/")
    .withEndpoint(AGE) // AGE, LIVENESS or AGE_LIVENESS
    .withHttpMethod("POST")
    .withHeader("X-Yoti-Auth-Id", "<YOTI_CLIENT_SDK_ID>")
    .withQueryParameter("secure", true)
    .withPayload(payload)
    .build();
...
If integrating into a non-browser client, we recommend contacting us [here](https://yoti.force.com/yotisupport/s/contactsupport) for additional support.
