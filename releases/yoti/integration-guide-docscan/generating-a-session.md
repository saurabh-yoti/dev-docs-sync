...
- [Creating the session](https://developers.yoti.com/yoti/generating-a-session#creating-the-session) broken down
...
Below is the complete example request snippet in different languages. The documentation continues to outline the full request in step by step detail.
...
Once you have added the Yoti SDK dependency to your project, you can use it to build and send your session as shown in the below code snippet:
...
When creating a session you will need to configure the below functions within your session:
...
| ---- | ---- | ---- | ---- | 
| [Document authenticity](https://developers.yoti.com/yoti/generating-a-session#document-authenticity) | 1x Document Resource | Asynchronous check | To determine the validity of the ID document.\n\nMust wait for the TextDataCheck, if there is one. | 
| [Liveness](https://developers.yoti.com/yoti/generating-a-session#liveness) | 1x Liveness Resource | Synchronous check | A check to assess whether the document scan is being performed by a real person and not someone wearing a mask or an automated system.\n\nRuns as soon as the FaceMap is uploaded.  Can be repeated until it succeeds, but only one successful _Check_ is allowed in each _Session._ | 
| [Face match](https://developers.yoti.com/yoti/generating-a-session#face-match) | 1x Doc Resource & 1x Liveness Resource | Asynchronous check | A request to assess whether the user's face matches the face on the ID document. The user's facial image is obtained from the liveness check, which must be performed first.\n\nMust wait for the TextDataCheck, if there is one. | 
| [Text Extraction](https://developers.yoti.com/yoti/generating-a-session#text-extraction) | 1 x Document Resource | Asynchronous task | Used to recover a failed TextExtractionTask | 
| [Client preferences](https://developers.yoti.com/yoti/generating-a-session#preferences) | N/A | N/A | Camera and pre set country options and where the user should be directed after the user journey (success/error redirect URL). | 
| [System preferences](https://developers.yoti.com/yoti/generating-a-session#preferences) | N/A | N/A | Doc Scan session length and user tracking. | 
| [Notifications](https://developers.yoti.com/yoti/generating-a-session#notifications) | N/A | N/A | An update notification when the session state changes, based on the selected subscription topics. | 
{% /table %}
...
The method sdkConfig builder is explained below. If not defined then they will be set to default (indicated):**
**
...
| ---- | ---- | ---- | ---- | 
| Allowed capture methods | withAllowsCameraAndUpload\n\nwithAllowsCamera | The user must use a camera to take a photo of their ID document, or the user can take a photo or has the option to upload an image of the document from file. | optional | 
| withPrimaryColour | #2d9fff(default), #ff0000 | Colours of the button provided in the frontend client as a hexadecimal value. | optional | 
...
| ---- | ---- | ---- | 
| withClientSessionTokenTtl | This is how long the full Yoti Doc Scan session is open for in seconds. This must be longer than 5 minutes. | optional | 
| withResourcesTtl | Retention period for uploaded documents/images in number of seconds. Default is one week (`60*60*24*7=604800`). This can be a minimum of 24 hours and must be at least 24 hours longer than the client_session_token_ttl. This starts once the `client_session_token_ttl` is completed. | optional | 
| withUserTrackingId | Allows the relying business backend to track the same user across multiple sessions. Note: Should not contain any personal identifiable information. | optional | 
{% /table %}
...
Yoti will return a list of countries listed by ISO country code. Each country will list an array of supported documents from that country.
