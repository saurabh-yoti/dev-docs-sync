...
Once you've set up your organisation account on the [Yoti Hub](https://hub.yoti.com/logout), you’re ready to start integrating with Yoti MyFace.

There is one step to follow to integrate MyFace:

[1) Create a MyFace request](https://developers.yoti.com/yoti-myface/integration-guide#create-a-request)
...
You can create three types of requests using the Yoti MyFace API.

{% table %}
| Endpoint | Description | 
| ---- | ---- | 
| [https://api.yoti.com/ai/v1/age](https://api.yoti.com/ai/v1/age) | Use Yoti's age estimation service. Head over [here](https://developers.yoti.com/yoti-age-scan) for more information. | 
| [https://api.yoti.com/ai/v1/age-antispoofing](https://api.yoti.com/ai/v1/age-antispoofing) | Use Yoti's age estimation service and the liveness check (MyFace). | 
| [https://api.yoti.com/ai/v1/antispoofing](https://api.yoti.com/ai/v1/antispoofing) | Use Yoti's liveness check as a standalone product. | 
{% /table %}
...
This endpoint will return a MyFace anti-spoofing result. Optionally if you include Yoti Age Scan, it will also include an age.

### Body explained

The request payload above should be in the below format. This JSON string will be the `IMG` parameter in the `withPayload` above code snippet.
...
| Response | Explained | 
| ---- | ---- | 
| Prediction - Real | Yoti MyFace has detected a real user. | 
| Prediction - Fake | Yoti MyFace has detected a spoof attempt. | 
...
| 400 | MIN_HEIGHT | The image height is incorrect. Image minimum width required is 300 pixels. | 
| 400 | MAX_HEIGHT | The image height is incorrect. Image minimum width required is 2000 pixels. | 
